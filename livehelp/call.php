...
if ($json) {
	...
	if (isset($_REQUEST['SESSION'])) {
		...
		if (sha1($decrypted) == $verify) {
			...
			if ($id > 0) {
				$query = sprintf("SELECT `status` FROM " . $_SETTINGS['TABLEPREFIX'] . "callback WHERE `id` = '%d' LIMIT 1", $id);
				...
				if ($status > 0) {
					$query = sprintf("UPDATE `" . $_SETTINGS['TABLEPREFIX'] . "callback` SET `status` = '%d' WHERE `id` = '%d' LIMIT 1", $status, $id);
...
if (isset($_REQUEST['NAME']) && isset($_REQUEST['EMAIL']) && isset($_REQUEST['COUNTRY']) && isset($_REQUEST['DIAL']) && isset($_REQUEST['TELEPHONE']) && isset($_REQUEST['MESSAGE'])) {
	...
	if (empty($name) || empty($email) || empty($message) || empty($country) || empty($telephone)) {
		...
		if (!preg_match('/^[\-!#$%&\'*+\\\\.\/0-9=?A-Z\^_`a-z{|}~]+@[\-!#$%&\'*+\\\\\/0-9=?A-Z\^_`a-z{|}~]+\.[\-!#$%&\'*+\\\\.\/0-9=?A-Z\^_`a-z{|}~]+$/', $email)) {
			...
			if ($security != $captcha && $_SETTINGS['SECURITYCODE'] == true && ((function_exists('imagepng') || function_exists('imagejpeg')) && function_exists('imagettftext'))) {
				...
				$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "callback(`datetime`, `name`, `email`, `country`, `timezone`, `dial`, `telephone`,`message`) VALUES(NOW(), '%s', '%s', '%s', '%s', '%s', '%s', '%s')", $SQL->escape($name), $SQL->escape($email), $SQL->escape($country), $SQL->escape($timezone), $SQL->escape($dial), $SQL->escape($telephone), $SQL->escape($message));
	...
	if (count($popular) > 0) {
		...
		$query = sprintf("SELECT `code`, `country`, `dial` FROM " . $_SETTINGS['TABLEPREFIX'] . "countries WHERE `code` = '%s' ORDER BY `country`", $where);
	...
	// Countries
	$query = 'SELECT `code`, `country`, `dial` FROM ' . $_SETTINGS['TABLEPREFIX'] . 'countries ORDER BY `country`';
	$row = $SQL->selectquery($query);
	while ($row) {
		if (is_array($row)) {
			$dial = '+' . $row['dial'];
