...
if (is_numeric($id)) {
	...
	$query = sprintf("SELECT `typing` FROM " . $_SETTINGS['TABLEPREFIX'] . "chats WHERE `id` = '%s' LIMIT 1", $id);
	...
	if (is_array($row)) {
		...
		$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "chats SET `typing` = '%d' WHERE `id` = '%s'", $result, $id);
