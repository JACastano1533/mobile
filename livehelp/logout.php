...
$query = sprintf("SELECT `id`, `username`, `active` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `id` = '%d' LIMIT 1", $chat);
...
if (is_array($row)) {
	...
	if (!empty($rating) && $rating > 0 && $rating < 6) {
		$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "chats SET `rating` = '%d' WHERE `id` = '%d' LIMIT 1", $rating, $chat);
		...
		if ($_SETTINGS['TRANSCRIPTVISITORALERTS'] == true) {
			...
			if ($active > 0) {
				...
				$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "messages (`chat`, `username`, `datetime`, `message`, `align`, `status`) VALUES ('%d', '%s', NOW(), '%s', '2', '-3')", $id, $SQL->escape($username), $SQL->escape($message));
		...
		$query = sprintf("SELECT `username`, `chats`.`request`, `active`, `custom`, UNIX_TIMESTAMP(`datetime`) AS `datetime` FROM " . $_SETTINGS['TABLEPREFIX'] . "chats AS chats, " . $_SETTINGS['TABLEPREFIX'] . "custom AS custom WHERE `chats`.`request` = `custom`.`request` AND `chats`.`id` = '%d' AND `reference` = 'WHMCS' LIMIT 1", $chat);
		$row = $SQL->selectquery($query);
		if (is_array($row)) {
			$username = $row['username'];
			$active = $row['active'];
			...
			if ($active > 0 && $custom > 0) {
				$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "chats SET `active` = '-1' WHERE `id` = '%d' LIMIT 1", $chat);
				$SQL->updatequery($query);
				
				$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `initiate` = '0' WHERE `id` = '%d'", $request);
				$SQL->updatequery($query);
				...
				if ($_SETTINGS['SERVERVERSION'] >= 3.90) {
					...
					$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "activity(`user`, `username`, `datetime`, `activity`, `duration`, `type`, `status`) VALUES ('%d', '%s', NOW(), '%s', UNIX_TIMESTAMP(NOW()) - %d, 9, 0)", $chat, $SQL->escape($username), $SQL->escape($message), $datetime);
					$SQL->insertquery($query);
				}
			...
			$query = sprintf("SELECT `username`, `request`, `active`, UNIX_TIMESTAMP(`datetime`) AS `datetime` FROM " . $_SETTINGS['TABLEPREFIX'] . "chats AS chats WHERE `id` = '%d' LIMIT 1", $chat);
			$row = $SQL->selectquery($query);
			if (is_array($row)) {
				$username = $row['username'];
				$request = $row['request'];
		...
		if (isset($_SETTINGS['AUTOEMAILTRANSCRIPT']) && $_SETTINGS['AUTOEMAILTRANSCRIPT'] != '') {
			...
			$query = sprintf("SELECT `requests`.*, `chats`.email FROM " . $_SETTINGS['TABLEPREFIX'] . "requests AS `requests`, " . $_SETTINGS['TABLEPREFIX'] . "chats AS `chats`  WHERE `chats`.`request` = `requests`.`id` AND `chats`.`id` = '%d' LIMIT 1", $chat);
			$row = $SQL->selectquery($query);
			if (is_array($row)) {
