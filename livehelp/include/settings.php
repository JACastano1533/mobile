...
// Total operators within each Online/Offline/BRB/Away status mode
if ((float)$_SETTINGS['SERVERVERSION'] >= 4.1) { // Multiple Device PUSH Supported
	...
	$query = "SELECT `username`, `status`, `department`, `device` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `refresh` > DATE_SUB(NOW(), INTERVAL " . $_SETTINGS['CONNECTIONTIMEOUT'] . " SECOND) OR `device` <> ''";
} else {
	$query = "SELECT `username`, `status`, `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `refresh` > DATE_SUB(NOW(), INTERVAL " . $_SETTINGS['CONNECTIONTIMEOUT'] . " SECOND)";
}
...
// Auto Initiate Chat
$initiate = false;
$query = sprintf("SELECT `initiate`, `path` FROM " . $_SETTINGS['TABLEPREFIX'] . "requests WHERE `id` = '%d'", $request);
...
if ($_SETTINGS['DEPARTMENTS'] == true)  {
	...
	if ((float)$_SETTINGS['SERVERVERSION'] >= 4.1) { // Multiple Device PUSH Supported
		$query = sprintf("SELECT DISTINCT `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users AS `users` LEFT JOIN " . $_SETTINGS['TABLEPREFIX'] . "devices AS `devices` ON `users`.`id` = `devices`.`user` WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`token` <> '' AND `status` = '1') ORDER BY `department`", $_SETTINGS['CONNECTIONTIMEOUT']);
	} elseif ((float)$_SETTINGS['SERVERVERSION'] >= 3.80) { // iPhone PUSH Supported
		$query = sprintf("SELECT DISTINCT `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`device` <> '' AND `status` = '1') ORDER BY `department`", $_SETTINGS['CONNECTIONTIMEOUT']);
	} else {
		$query = sprintf("SELECT DISTINCT `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1' ORDER BY `department`", $_SETTINGS['CONNECTIONTIMEOUT']);
	}
	$rows = $SQL->selectall($query);
	if (is_array($rows)) {
		$departments = array();
		foreach ($rows as $key => $row) {
			if (is_array($row)) {
				$depmnts = explode(';',  $row['department']);
				if (is_array($depmnts)) {
...
if ($chat > 0) {
	if ($request > 0) {
		$query = sprintf("SELECT `id`, `username`, `email`, `department`, `active` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` AS `chats` WHERE (`id` = '%d' OR `request` = '%d') LIMIT 1", $chat, $request);
	} else {
		$query = sprintf("SELECT `id`, `username`, `email`, `department`, `active` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` AS `chats` WHERE `id` = '%d' LIMIT 1", $chat, $request);
	}
	$row = $SQL->selectquery($query);
	if (is_array($row)) {
		$active = $row['active'];
...
if ($request > 0) {
	...
	// Chat Session
	if ($chat == 0 && $request > 0) {
		$query = sprintf("SELECT `id`, `username`, `email`, `department`, `active` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `request` = '%d' LIMIT 1", $request);
		$row = $SQL->selectquery($query);
		if (is_array($row)) {
...
if ($json) {
	...
	if ((float)$_SETTINGS['SERVERVERSION'] >= 4.1) { // Multiple Device PUSH Supported
		$query = sprintf("SELECT `users`.`id` AS `id`, `firstname`, `lastname`, `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users AS `users` LEFT JOIN " . $_SETTINGS['TABLEPREFIX'] . "devices AS `devices` ON `users`.`id` = `devices`.`user` WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`token` <> '' AND `status` = '1')", $_SETTINGS['CONNECTIONTIMEOUT']);
	} elseif ((float)$_SETTINGS['SERVERVERSION'] >= 3.80) { // iPhone PUSH Supported
		$query = sprintf("SELECT `id`, `firstname`, `lastname`, `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`device` <> '' AND `status` = '1')", $_SETTINGS['CONNECTIONTIMEOUT']);
	} else {
		$query = sprintf("SELECT `id`, `firstname`, `lastname`, `department` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1'", $_SETTINGS['CONNECTIONTIMEOUT']);
	}
	$rows = $SQL->selectall($query);
	if (is_array($rows)) {
		if (!empty($department)) {
			...
			if (count($rows)) {
				$initiateop = $operators[array_rand($operators)];
			}
		} else {
			$initiateop = $rows[array_rand($rows)];
		}
	}
	if (is_array($initiateop)) {
		$embeddedinitate = array('id' => (int)$initiateop['id'], 'name' => $initiateop['firstname'] . ' ' . $initiateop['lastname'], 'department' => $initiateop['department']);
	}
...
LiveHelp.department = '<?php echo($depmnt); ?>';
