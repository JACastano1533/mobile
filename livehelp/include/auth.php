...
if (isset($_COOKIE['LiveHelpOperator'])) {
	...
	if (!empty($current_user_id) && !empty($password))  {
		$query = sprintf("SELECT `username`, `department`, `privilege`, `datetime` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `id` = '%d' AND `password` = '%s'", $SQL->escape($current_user_id), $SQL->escape($password));
...
elseif (!empty($_REQUEST['USERNAME']) && !empty($_REQUEST['PASSWORD'])) {
	...
	$query = sprintf("SELECT `id`, `username`, `department`, `privilege` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `username` LIKE BINARY '%s' AND `password` = '%s'", $SQL->escape($username), $SQL->escape($password));
	$row = $SQL->selectquery($query);
	if (is_array($row)) {
		...
		setcookie('LiveHelpOperator', $data, false, '/', $_SETTINGS['COOKIEDOMAIN'], 0);
	}
	else {
