...
if ($GUEST['VISITOR'] > 0) {
	...
	$query = sprintf("SELECT `path` FROM " . $_SETTINGS['TABLEPREFIX'] . "requests WHERE `id` = '%d' LIMIT 1", $GUEST['VISITOR']);
	...
	if (is_array($row)) {
		...
		if ($page != trim(end($previouspath))) {
			$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `request` = NOW(), `url` = '%s', `path` = '%s; %s', `status` = '0' WHERE `id` = '%d'", $SQL->escape($url), $SQL->escape($path), $SQL->escape($page), $GUEST['VISITOR']);
			$SQL->updatequery($query);
			$totalpages = count($previouspath) + 1;
			
			if ($_SETTINGS['TRANSCRIPTVISITORALERTS'] == true) {
				if ($GUEST['CHAT'] > 0) {
					$query = sprintf("SELECT `id`, `username`, `active` FROM " . $_SETTINGS['TABLEPREFIX'] . "chats WHERE `id` = '%d' LIMIT 1", $GUEST['CHAT']);
					$row = $SQL->selectquery($query);
					if (is_array($row)) {
						$id = $row['id'];
						...
						if ($active > 0) {
							...
							$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "messages (`chat`, `username`, `datetime`, `message`, `align`, `status`) VALUES ('%d', '', NOW(), '%s', '2', '-2')", $id, $message);
			...
			$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `request` = NOW(), `url` = '%s', `status` = '0' WHERE `id` = '%d'", $SQL->escape($url), $GUEST['VISITOR']);
			$SQL->updatequery($query);
...
var domain = '<?php echo($_SETTINGS['COOKIEDOMAIN']); ?>';
...
function LoadXMLTracking(initiateResult) {
	...
	if (timeElapsed < idleTimeout) {
		...
		TrackingTimer = window.setTimeout(function(){ 
			LoadXMLTracking();
		}, <?php echo($_SETTINGS['VISITORREFRESH'] * 1000); ?>);
	}
...
function floatRefresh() {
	...
	window.clearTimeout(TrackingTimer);
	if (countTracker == <?php echo($_SETTINGS['VISITORREFRESH'] * 1000); ?>) {
...
$query = "SELECT `username`, `status`, `department`, `device` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE `refresh` > DATE_SUB(NOW(), INTERVAL " . $_SETTINGS['CONNECTIONTIMEOUT'] . " SECOND)";
...
if ($tracker == true && $_SETTINGS['VISITORTRACKING'] == true) {
	...
	if ($GUEST['VISITOR'] > 0) {
		$query = sprintf("SELECT `initiate` FROM " . $_SETTINGS['TABLEPREFIX'] . "requests WHERE `id` = '%d'", $GUEST['VISITOR']);
		$row = $SQL->selectquery($query);
		if (is_array($row)) {
...
<?php
	...
	$window = preg_replace('/[^a-z]/i', '', $_SETTINGS['COOKIEDOMAIN']);
?>
		return false;
	} else {
