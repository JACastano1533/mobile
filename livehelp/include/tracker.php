...
if ($GUEST['VISITOR'] > 0) {
	...
	$query = sprintf("SELECT `initiate`, `status` FROM " . $_SETTINGS['TABLEPREFIX'] . "requests WHERE `id` = '%d' LIMIT 1", $GUEST['VISITOR']);
	...
	if (is_array($row)) {
		...
		if ($initiate != '') {
			...
			if ($initiate == 'Opened') {
				...
				$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `refresh` = NOW(), `initiate` = '-1' WHERE `id` = '%d'", $GUEST['VISITOR']);
				$SQL->updatequery($query);
			}
			elseif ($initiate == 'Accepted') {
				// Update request flag to show that the guest user ACCEPTED the Online Chat Request
				$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `refresh` = NOW(), `initiate` = '-2' WHERE `id` = '%d'", $GUEST['VISITOR']);
				$SQL->updatequery($query);
			}
			elseif ($initiate == 'Declined') {
				// Update request flag to show that the guest user DENIED the Online Chat Request
				$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `refresh` = NOW(), `initiate` = '-3' WHERE `id` = '%d'", $GUEST['VISITOR']);
				$SQL->updatequery($query);
			}
		...
		if ($url == '' && $title == '') {  // Update current page time
			$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `refresh` = NOW(), `status` = '%s' WHERE `id` = '%d'", $SQL->escape($request_status), $GUEST['VISITOR']);
			$SQL->updatequery($query);
		}
		else {  // Update current page details
			$query = sprintf("UPDATE " . $_SETTINGS['TABLEPREFIX'] . "requests SET `refresh` = NOW(), `request` = NOW(), `url` = '%s', `title` = '%s', `status` = '0' WHERE `id` = '%d'", $SQL->escape($url), $SQL->escape($title), $GUEST['VISITOR']);
			$SQL->updatequery($query);
		}
	...
	if ($width != '' && $height != '' && $url != '') {
		...
		if (file_exists('../plugins/maxmind/GeoLiteCity.dat') && $_SETTINGS['SERVERVERSION'] >= 3.90) {
			...
			if (!empty($record)) {
				...
				$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "requests(`ipaddress`, `useragent`, `resolution`, `city`, `state`, `country`, `datetime`, `request`, `refresh`, `url`, `title`, `referrer`, `path`, `initiate`, `status`) VALUES('%s', '%s', '%s x %s', '%s', '%s', '%s', NOW(), NOW(), NOW(), '%s', '%s', '%s', '%s', '0', '0')", $SQL->escape($ipaddress), $SQL->escape($useragent), $SQL->escape($width), $SQL->escape($height), $SQL->escape($city), $SQL->escape($state), $SQL->escape($country), $SQL->escape($url), $SQL->escape($title), $SQL->escape($referrer), $SQL->escape($page));
				$GUEST['VISITOR'] = $SQL->insertquery($query);
			} else {
			
				$query = sprintf("INSERT INTO " . $_SETTINGS['TABLEPREFIX'] . "requests(`ipaddress`, `useragent`, `resolution`, `city`, `state`, `country`, `datetime`, `request`, `refresh`, `url`, `title`, `referrer`, `path`, `initiate`, `status`) VALUES('%s', '%s', '%s x %s', '', '', '%s', NOW(), NOW(), NOW(), '%s', '%s', '%s', '%s', '0', '0')", $SQL->escape($ipaddress), $SQL->escape($useragent), $SQL->escape($width), $SQL->escape($height), $SQL->escape($country), $SQL->escape($url), $SQL->escape($title), $SQL->escape($referrer), $SQL->escape($page));
				$GUEST['VISITOR'] = $SQL->insertquery($query);
