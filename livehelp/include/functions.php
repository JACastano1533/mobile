	...
	if (is_array($multi_departments)) {
		...
		if ($length > 1) {
			while ($i < $length):
				...
				if ($i == 0) {
					$sql = sprintf("( `department` = '%s'", $SQL->escape($department));
				}
				elseif ($i > 0 && $i < $length - 1) {
					$sql .= sprintf(" OR `department` = '%s'", $SQL->escape($department));
				}
				elseif ($i == $length - 1) {
					$sql .= sprintf(" OR `department` = '%s' OR `department` = '' )", $SQL->escape($department));
				}
				$i++;
			endwhile;
		}
			...
			$sql = sprintf("( `department` = '%s' OR `department` = '' )", $SQL->escape($department));
		}
	}
	else {
		$department = trim($department);
