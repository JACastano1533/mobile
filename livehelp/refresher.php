...
$query = sprintf("SELECT `typing` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `id` = '%d' LIMIT 1", $chat);
...
if (is_array($row)) {
	...
	$query = sprintf("UPDATE `" . $_SETTINGS['TABLEPREFIX'] . "chats` SET `typing` = '%d' WHERE `id` = '%d'", $result, $chat);
	$SQL->updatequery($query);
	
}

// Check if an operator has accepted the chat request
$query = sprintf("SELECT `username`, `active`, `department`, `datetime` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `id` = '%d'", $chat);
$row = $SQL->selectquery($query);
if (is_array($row)) {
...
if ($active > 0) {
	...
	// Operator Full Name
	$query = sprintf("SELECT `id`, `username`, `firstname`, `lastname`, `department` FROM `" . $_SETTINGS['TABLEPREFIX'] . "users` WHERE `id` = '%d' LIMIT 1", $active);
	$row = $SQL->selectquery($query);
	if (is_array($row)) {
		$operator = $row;
	}
	...
	if ($message > 0) {
		// New Messages
		$query = sprintf("SELECT `id`, `datetime`, `username`, `message`, `align`, `status` FROM `" . $_SETTINGS['TABLEPREFIX'] . "messages` WHERE `chat` = '%d' AND `status` >= '0' AND `id` > '%d' ORDER BY `datetime`", $chat, $message);
	} else {
		// All Messages except PUSH
		$query = sprintf("SELECT `id`, `datetime`, `username`, `message`, `align`, `status` FROM `" . $_SETTINGS['TABLEPREFIX'] . "messages` WHERE `chat` = '%d' AND `status` >= '0' AND `status` <> '4' ORDER BY `datetime`", $chat);
	}
	$messagerows = $SQL->selectall($query);
	if (is_array($messagerows)) {

		// Total Operators
		$query = sprintf("SELECT DISTINCT `username` FROM " . $_SETTINGS['TABLEPREFIX'] . "messages WHERE `chat` = '%d' AND `status` > '0'", $chat);
					...
					if ((unixtimestamp($row['datetime']) - unixtimestamp($datetime)) > 0) {
						if ($operators > 1 && in_array($row['username'], $operators)) {
							...
							if (($operator['username'] != $row['username']) && $status > 0) {
								...
								$query = sprintf("SELECT `id`, `username`, `firstname`, `lastname` FROM `" . $_SETTINGS['TABLEPREFIX'] . "users` WHERE `username` = '%d' LIMIT 1", $SQL->escape($row['username']));
								$row = $SQL->selectquery($query);
								if (is_array($row)) {
									$first = $row['firstname'];
									$last = $row['lastname'];
...
$timeout = $_SETTINGS['CONNECTIONTIMEOUT'] * 2;
$query = sprintf("SELECT `id` FROM `" . $_SETTINGS['TABLEPREFIX'] . "users` WHERE `refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `id` = '%d' LIMIT 1", $timeout, $active);
$row = $SQL->selectquery($query);
if (!is_array($row)) {
	$connection = false;
}
...
if ($active > 0 && $message == 0) {
	...
	if (isset($operator) && is_array($operator)) {
		...
		$javascript = "jQuery(document).trigger('LiveHelp.Connected', [{$operator['id']}, '{$name}', '{$depmnt}']);";
...
$query = sprintf("SELECT `typing` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `id` = '%d'", $chat);
$row = $SQL->selectquery($query);
if (is_array($row)) {
	$typing = $row['typing'];
...
$query = sprintf("UPDATE `" . $_SETTINGS['TABLEPREFIX'] . "chats` SET `refresh` = NOW() WHERE `id` = '%d'", $chat);
$SQL->updatequery($query);
