...
if ($installed == true) {
	...
	if (empty($_REQUEST['ERROR'])) {
		if ((float)$_SETTINGS['SERVERVERSION'] >= 4.1) { // Multiple Device PUSH Supported
			$query = sprintf("SELECT `users`.`id` AS `id`, `department`, `devices`.`token` AS `device` FROM " . $_SETTINGS['TABLEPREFIX'] . "users AS `users` LEFT JOIN " . $_SETTINGS['TABLEPREFIX'] . "devices AS `devices` ON `users`.`id` = `devices`.`user` WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`token` <> '' AND `status` = '1')", $_SETTINGS['CONNECTIONTIMEOUT']);
		} elseif ((float)$_SETTINGS['SERVERVERSION'] >= 3.80) { // iPhone PUSH Supported
			$query = sprintf("SELECT `id`, `department`, `device` FROM " . $_SETTINGS['TABLEPREFIX'] . "users WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`device` <> '' AND `status` = '1')", $_SETTINGS['CONNECTIONTIMEOUT']);
		} else {
...
// Login Details
if ($chat > 0) {
	$query = sprintf("SELECT `username`, `email`, `department` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE `id` = '%d' LIMIT 1", $chat);
...
$connected = false;
if ($chat > 0 || $request > 0) {
	$query = sprintf("SELECT `username`, `email`, `server`, `department`, `active` FROM `" . $_SETTINGS['TABLEPREFIX'] . "chats` WHERE (`id` = '%d' OR `request` = '%d') AND `active` > 0 LIMIT 1", $chat, $request);
	$row = $SQL->selectquery($query);
	if (is_array($row)) {
		$connected = true;
	}
